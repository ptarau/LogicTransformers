...
def unifiable(code,i,G, trail,htop,ttop) :
  clause = code[i]
  c = clause.c
  hb, ctag = c
  h=left(hb)
  ok=unify(h,G,trail,htop)
  unwind(trail,ttop)
  return ok
...
def interp() :
  ...
  def loop(G,k):
    ...
    while i < l :
      ...
      if not unifiable(code,i,G, trail,htop,ttop) :
        i+=1;
        continue

      hb,thb=activate(code,i)
      assert PAIR==thb
      h=left(hb)
      assert h[0] >= htop
      assert G[0] < htop
      if unify(G,h,trail,htop):
        b,tb=deref(right(hb))
        if VAR==tb:
          yield p(left(goal[0]))
