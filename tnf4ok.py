...
def unify(x,y,trail,vars) :
  ...
  while ustack :
    ...
    if x1 is x2:
      ...
      x1[0]=x2
      if not in_vars(x1,vars):
        trail.append(x1)
    elif VAR==t2:
      x2[0]=x1
      if not in_vars(x2,vars):
        trail.append(x2)
    elif t1!=t2 :
      return False
    elif t1==CONST:
      ...
      for instr in clause :
        ...
        if op=='u':
          ...
          if VAR==oldt :
            ...
            r=unify(c[1],old[0],trail,vars) and \
              unify(c[2],old[1],trail,vars)
          if not r :
            unwind(trail,ttop)
            break
        elif op=='d':
