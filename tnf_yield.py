    ...
    while i < l :
      ...
      for instr in clause :
        ...
        if op=='u' or op=='b':
          ...
          c[1][0]=G # first var - always unbound
          trail.append(c[1])
        else: #p(..)
          NewG,tg=deref(c[1])
          if VAR == tg : # no more Cont left to execute
            yield iron(goal) # return dereferened copy of goal
          else: # moving forward on this AND branch
            #assert PAIR==tg
            yield from loop(NewG)
          unwind(trail, ttop) # gives a chance to other clauses
